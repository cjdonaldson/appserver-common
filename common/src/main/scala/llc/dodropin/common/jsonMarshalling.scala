...
case class JsonMarshalling[A]() {
  ...
  implicit final def unmarshaller(implicit
  ...
  implicit final def marshaller(implicit
      jsonEncoder: Encoder[A]
  ): ToEntityMarshaller[A] =
    Marshaller.withFixedContentType(`application/json`)((a: A) => HttpEntity(`application/json`, a.asJson.noSpaces))
}
